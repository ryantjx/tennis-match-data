...
Status: approved direction for v4; implementation has not started.
...
The current v3.2 automation is operational, but it automates an inadequate date
contract.
...
- v3.2 makes terminal `date` non-null by using the exact match day when
  available and otherwise falling back to the tournament start date. The
  resulting approximately 1.74 million completed rows must not be described as
...
- The current Wikimedia fixture collector discovers draw slots, but ordinarily
  ...
  expected.
- The repository has no implemented tennis-data.co.uk adapter, official
  order-of-play adapter, or general exact-date observation pipeline.

Passing validation currently proves that files satisfy the v3.2 contract. It
does not prove that a completed row's `date` is the day on which the match was
played. v4 changes that definition rather than weakening the word "exact."
...
v3.2 is frozen as an immutable legacy event-date snapshot. Its documentation
must state that completed `date` may be the tournament start date. It remains
available during the measured v4 transition, but it is not an exact-date
dataset.
...
- Mark v3.2 documentation as legacy and accurate about its date fallback.
