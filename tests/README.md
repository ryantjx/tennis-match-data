...
| `test_scripts.py` | Release-asset preconditions, GitHub upload arguments through a stubbed `gh`, and the no-change behavior of the automated data commit script. |
...
isolation, prove older checksums unchanged, and verify local and published assets. The
weekly audit validates all local history, checks upstream revisions for the previous and
current result years plus current/next fixtures, and opens a review-only PR only for
validated semantic changes.
