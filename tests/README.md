...
- `fixture_dataset.py`: a tiny offline, source-attributed release input used
  by release and CLI tests instead of committed generated data.
...
Release CI builds the same pinned offline fixture twice, compares every byte,
and proves that preview coverage cannot pass `--require-complete`. There are
no browser, Node, frontend, Pages, automated data-commit, or data-PR tests.
